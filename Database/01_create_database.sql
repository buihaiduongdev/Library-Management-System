...
CREATE TABLE SACH (
    Id INT PRIMARY KEY IDENTITY(1, 1),
    MaSach VARCHAR(50) NULL,  -- Đã thay đổi từ NOT NULL thành NULL
    TenSach NVARCHAR(255) NOT NULL,
    NamXuatBan INT NOT NULL CHECK (NamXuatBan > 0),
    GiaSach DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
    AnhBia VARBINARY(MAX),
    MaNXB VARCHAR(50) NULL REFERENCES NHA_XUAT_BAN(MaNXB) ON DELETE NO ACTION,  
    MaTacGia VARCHAR(50) NULL REFERENCES TAC_GIA(MaTacGia) ON DELETE NO ACTION,  
    MaTheLoai VARCHAR(50) NULL REFERENCES THE_LOAI(MaTheLoai) ON DELETE NO ACTION  
);
GO
...
CREATE TABLE The_Nhap (
    ...
    MaSach VARCHAR(50) NULL REFERENCES SACH(MaSach) ON DELETE NO ACTION  
);
GO
	
CREATE TABLE Kho_Sach (
    Id INT PRIMARY KEY IDENTITY(1, 1),  
    MaSach VARCHAR(50) NULL REFERENCES SACH(MaSach) ON DELETE CASCADE,  
